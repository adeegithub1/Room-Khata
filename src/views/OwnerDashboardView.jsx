...
function BottomNav({ active, onNav }) {
  return (
    <nav
      style={{
        flexShrink: 0,                 /* stays as a flex child, never scrolls away */
        background: "rgba(255,255,255,0.97)",
        backdropFilter: "blur(24px)",
        WebkitBackdropFilter: "blur(24px)",
        borderTop: `1.5px solid ${B.border}`,
        boxShadow: "0 -4px 24px rgba(30,27,75,0.10)",
        paddingTop: 10,
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── Premium header — sticky, collapses on scroll ── */}
        <FinanceHeader
          ownerName={ownerName}
          rooms={rooms}
          loading={loading}
        ...
        {/* ── Scrollable body — flex-1 so it fills space between header and nav ── */}
        <div
          ref={scrollRef}
          onScroll={handleScroll}
          className="flex-1 overflow-y-auto"
          style={{ background: B.cream, paddingBottom: 8 }}
        >
        ...
        {/* ── Bottom nav — flex sibling, always visible ── */}
        <BottomNav active={activeNav} onNav={handleNav} />

      </div>

      {/* Toast stack — rendered outside flex column so it overlays everything */}
