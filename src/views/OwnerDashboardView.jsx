...
const dueFor = r => Math.max(
  0,
  r?.balanceDue != null
    ? Number(r.balanceDue || 0)
    : Number(r?.rent || 0) + Number(r?.electricityBill || 0) - Number(r?.amountPaid || 0)
);
const totalFor = r => Number(r?.rent || 0) + Number(r?.electricityBill || 0);
...
/* ─── Room Card ──────────────────────────────────────────── */
function RoomCard({ room, onToggle, onReceive, onEdit, onInvite, onDelete, onAddBill, onAssign, onViewDetail }) {
        ...
        <div style={{marginTop:8,display:"flex",flexDirection:"column",gap:5}}
          ...
          {!vacant ? (
            status==="pending_verification" ? (
              ...
              <>
                <button onClick={()=>status==="paid" ? onToggle(room.id,status) : onReceive(room)}
                  style={{width:"100%",padding:"7px 4px",borderRadius:10,border:"none",cursor:"pointer",
...
/* ─── Building Group ─────────────────────────────────────── */
function BuildingGroup({ bid, name, rooms, onToggle, onReceive, onEdit, onAddRoom, onInvite, onDelete, onAddBill, onAssign, onViewDetail }) {
      ...
      <div style={{display:"flex",gap:10,overflowX:"auto",paddingBottom:4,scrollbarWidth:"none"}}>
        {rooms.map(r=>(
          <div key={r.id} style={{minWidth:140,maxWidth:140}}>
            <RoomCard room={r} onToggle={onToggle} onReceive={onReceive} onEdit={onEdit}
...
function ReceivePaymentSheet({ room, onClose, toast }) {
  const [amount, setAmount] = useState(String(dueFor(room) || totalFor(room) || ""));
  const [method, setMethod] = useState(room.status === "pending_verification" ? "upi" : "cash");
  const [note, setNote] = useState("");
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  const total = totalFor(room);
  const alreadyPaid = Number(room.amountPaid || 0);
  const due = dueFor(room);

  const save = async e => {
    e.preventDefault();
    const paidNow = parseInt(amount, 10) || 0;
    if (paidNow <= 0) { setErr("Enter a payment amount."); return; }
    setBusy(true); setErr("");
    try {
      const now = new Date();
      const newPaid = room.status === "pending_verification" ? Math.max(alreadyPaid, paidNow) : alreadyPaid + paidNow;
      const nextBalance = Math.max(0, total - newPaid);
      const nextStatus = nextBalance === 0 ? "paid" : "partial";
      await updateDoc(doc(db, "rooms", room.id), {
        status: nextStatus,
        amountPaid: newPaid,
        balanceDue: nextBalance,
        paidDate: nextStatus === "paid" ? now.toISOString() : room.paidDate || null,
      ...
      await addDoc(collection(db, "rentLedger"), {
        ...
        type: room.status === "pending_verification" ? "verified_payment" : "owner_payment",
        method,
        amount: paidNow,
        rent: room.rent || 0,
      ...
      toast(nextStatus === "paid" ? "Payment marked paid." : `Partial payment saved. Balance ${inr(nextBalance)}.`);
      onClose();
    } catch (e) {
      setErr(e.message || "Payment update failed.");
    }
    setBusy(false);
  };

  return (
    <Sheet onClose={onClose} title={`Receive Payment - Room ${room.roomNo}`}>
      <div style={{background:C.bg,borderRadius:16,padding:"14px",border:`1.5px solid ${C.bdr}`,marginBottom:14}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div>
            <p style={{fontSize:10,fontWeight:700,color:C.t3,textTransform:"uppercase"}}>Total</p>
            <p style={{fontWeight:900,fontSize:18,color:C.t1,fontFamily:"'JetBrains Mono',monospace"}}>{inr(total)}</p>
          </div>
          <div>
            <p style={{fontSize:10,fontWeight:700,color:C.t3,textTransform:"uppercase"}}>Due Now</p>
            <p style={{fontWeight:900,fontSize:18,color:"#E11D48",fontFamily:"'JetBrains Mono',monospace"}}>{inr(due)}</p>
          </div>
        </div>
      </div>
      <form onSubmit={save}>
        <SInput label="Amount received" type="number" value={amount} onChange={setAmount} placeholder="8000" min="1" required/>
        <div style={{marginBottom:13}}>
          <label style={{display:"block",fontSize:11,fontWeight:700,color:C.vi,textTransform:"uppercase",letterSpacing:".08em",marginBottom:6}}>Payment Method</label>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7}}>
            {["cash","upi","bank"].map(m=>(
              <button key={m} type="button" onClick={()=>setMethod(m)}
                style={{padding:"9px 6px",borderRadius:12,border:`1.5px solid ${method===m?C.ind:C.bdr}`,
                  background:method===m?C.indLight:C.card,color:method===m?C.ind:C.t2,
                  cursor:"pointer",fontWeight:800,fontSize:12,textTransform:"uppercase"}}>
                {m}
              </button>
            ))}
          </div>
        </div>
        <SInput label="Note / UTR optional" value={note} onChange={setNote} placeholder="Cash, UTR, adjustment note"/>
        <ErrBox msg={err}/>
        <SBtn loading={busy} label="Save Payment" grad={G.emerald}/>
        <div style={{height:8}}/>
      </form>
    </Sheet>
  );
...
function AddBillSheet({ room, onClose, toast }) {
  ...
  const save = async e => {
    e.preventDefault();
    const bill = parseInt(amount, 10) || 0;
    setBusy(true); setErr("");
    try {
      const now = new Date();
      const total = Number(room.rent || 0) + bill;
      const paid = Number(room.amountPaid || 0);
      const balanceDue = Math.max(0, total - paid);
      const status = !room.tenantName?.trim() ? "pending" : balanceDue === 0 ? "paid" : paid > 0 ? "partial" : "pending";
      await updateDoc(doc(db, "rooms", room.id), {
        electricityBill: bill,
        balanceDue,
        status,
        billMonth: monthKey(now),
        billNote: note.trim(),
      ...
      toast("Electricity bill updated.");
      onClose();
    } catch (e) {
      setErr(e.message || "Could not save bill.");
    }
    setBusy(false);
  };

  return (
    <Sheet onClose={onClose} title={`Add Bill - Room ${room.roomNo}`}>
      <form onSubmit={save}>
        <SInput label="Electricity amount" type="number" value={amount} onChange={setAmount} placeholder="750" min="0" required/>
        <SInput label="Note optional" value={note} onChange={setNote} placeholder="Meter reading, shared meter, etc."/>
        <ErrBox msg={err}/>
        <SBtn loading={busy} label="Update Due Balance" grad={G.amber}/>
        <div style={{height:8}}/>
      </form>
    </Sheet>
  );
...
function AssignTenantSheet({ room, onClose, toast }) {
  ...
  const save = async e => {
    ...
    const rentValue = parseInt(rent, 10) || 0;
    setBusy(true); setErr("");
    try {
      await updateDoc(doc(db, "rooms", room.id), {
        tenantName: name.trim(),
        tenantPhone: phone.replace(/\D/g, "").slice(0, 10),
        rent: rentValue,
        ...
        status: "pending",
        amountPaid: 0,
        balanceDue: rentValue + Number(room.electricityBill || 0),
        assignedAt: new Date().toISOString(),
      });
      toast("Tenant assigned.");
      onClose();
    } catch (e) {
      setErr(e.message || "Could not assign tenant.");
    }
    setBusy(false);
  };
      ...
      <form onSubmit={save}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          <button type="button" onClick={pickPhoto}
            style={{width:64,height:64,borderRadius:18,border:`1.5px solid ${C.bdr}`,background:C.bg,
              overflow:"hidden",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            {photo ? <img src={photo} alt="tenant" style={{width:"100%",height:"100%",objectFit:"cover"}}/> : <i className="fa-solid fa-camera" style={{color:C.t3}}/>}
          </button>
          <div>
            <p style={{fontWeight:800,fontSize:13,color:C.t1}}>Tenant photo</p>
            <p style={{fontSize:11,color:C.t3}}>Optional, stored with the room record.</p>
          </div>
        </div>
        <SInput label="Tenant name" value={name} onChange={setName} placeholder="Ravi Kumar" required/>
        <SInput label="WhatsApp number" value={phone} onChange={v=>setPhone(v.replace(/\D/g,"").slice(0,10))} placeholder="10 digit mobile"/>
        <SInput label="Monthly rent" type="number" value={rent} onChange={setRent} placeholder="8000" min="0"/>
        <SInput label="Security deposit" type="number" value={deposit} onChange={setDeposit} placeholder="16000" min="0"/>
        <SInput label="Aadhaar / ID note optional" value={aadhaar} onChange={setAadhaar} placeholder="Last 4 digits or document note"/>
        <ErrBox msg={err}/>
        <SBtn loading={busy} label="Assign Tenant" grad={G.violet}/>
        <div style={{height:8}}/>
      </form>
    </Sheet>
  );
...
function RoomDetailSheet({ room, buildings, onClose, onEdit, onToggle, onAddBill, onAssign, onInvite, onDelete, onReceive }) {
  const vacant = !room.tenantName?.trim();
  const total = totalFor(room);
  const due = dueFor(room);
  const rows = [
    ["Building", buildings[room.buildingId]?.name || "Unassigned"],
    ["Room No.", room.roomNo || "-"],
    ["Tenant", room.tenantName || "Vacant"],
    ["Phone", room.tenantPhone ? `+91 ${room.tenantPhone}` : "-"],
    ["Rent", inr(room.rent || 0)],
    ["Electricity", inr(room.electricityBill || 0)],
    ["Deposit", inr(room.securityDeposit || 0)],
    ["Total", inr(total)],
    ["Balance", inr(due)],
    ["Connection Code", room.connectionCode || "-"],
  ];

  return (
    <Sheet onClose={onClose} title={`Room ${room.roomNo} Details`}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
        <div style={{width:72,height:72,borderRadius:20,background:C.indLight,overflow:"hidden",
          display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          {room.tenantPhoto
            ? <img src={room.tenantPhoto} alt={room.tenantName || "tenant"} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            : <span style={{fontWeight:900,fontSize:22,color:C.ind}}>{vacant ? room.roomNo : init(room.tenantName)}</span>}
        </div>
        <div style={{flex:1}}>
          <p style={{fontWeight:900,fontSize:18,color:C.t1}}>{vacant ? "Vacant room" : room.tenantName}</p>
          <p style={{fontSize:12,color:C.t2}}>Status: {vacant ? "vacant" : room.status || "pending"}</p>
          <p style={{fontSize:13,fontWeight:900,color:due>0?"#E11D48":"#059669",fontFamily:"'JetBrains Mono',monospace",marginTop:4}}>
            Due {inr(due)}
          </p>
        </div>
      </div>
      <div style={{background:C.bg,borderRadius:16,padding:"6px 14px",border:`1.5px solid ${C.bdr}`,marginBottom:14}}>
        {rows.map(([label,value])=>(
          <div key={label} style={{display:"flex",justifyContent:"space-between",gap:12,padding:"9px 0",borderBottom:`1px solid ${C.bdr2}`}}>
            <span style={{fontSize:12,color:C.t2}}>{label}</span>
            <span style={{fontSize:12,fontWeight:800,color:C.t1,textAlign:"right"}}>{value}</span>
          </div>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        {vacant ? (
          <>
            <button onClick={()=>onAssign(room)} style={{padding:"12px",borderRadius:13,border:"none",background:C.ind,color:"white",fontWeight:800,cursor:"pointer"}}>Assign</button>
            <button onClick={()=>onInvite(room)} style={{padding:"12px",borderRadius:13,border:"none",background:C.indLight,color:C.ind,fontWeight:800,cursor:"pointer"}}>Invite</button>
          </>
        ) : (
          <>
            <button onClick={()=>onReceive(room)} style={{padding:"12px",borderRadius:13,border:"none",background:C.ind,color:"white",fontWeight:800,cursor:"pointer"}}>Receive</button>
            <button onClick={()=>onAddBill(room)} style={{padding:"12px",borderRadius:13,border:"none",background:"#FEF3C7",color:"#B45309",fontWeight:800,cursor:"pointer"}}>Add Bill</button>
            <button onClick={()=>onEdit(room)} style={{padding:"12px",borderRadius:13,border:"none",background:C.bg,color:C.vi,fontWeight:800,cursor:"pointer"}}>Edit</button>
            <button onClick={()=>onToggle(room.id,room.status)} style={{padding:"12px",borderRadius:13,border:"none",background:C.bg,color:C.t2,fontWeight:800,cursor:"pointer"}}>
              {room.status==="paid" ? "Undo Paid" : "Mark Paid"}
            </button>
          </>
        )}
      </div>
      <button onClick={()=>onDelete("room",room.id,`Room ${room.roomNo}`)}
        style={{width:"100%",marginTop:10,padding:"12px",borderRadius:13,border:"none",background:"#FEE2E2",color:"#DC2626",fontWeight:800,cursor:"pointer"}}>
        Delete Room
      </button>
      <div style={{height:8}}/>
    </Sheet>
...
export default function OwnerDashboardView() {
  ...
  const [receiveRoom, setReceiveRoom] = useState(null);
    ...
    <>
      ...
      <AnimatePresence>
        {viewRoom&&<RoomDetailSheet key="rd"
          ...
          onReceive={r=>{setViewRoom(null);setReceiveRoom(r);}}
