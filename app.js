...
window.shareRoomLink = function(roomId, roomNo) {
    // Look up the room's connectionCode from cached roomsData
    const room = roomsData.find(r => r.id === roomId);
    const code = room?.connectionCode || '(code not set)';
...
window.addEventListener('popstate', function(e) {
    ...
    // If state carries a returnTo view, go there (back from sub-page → home)
    if (state && state.returnTo && state.returnTo !== 'view-login') {
        window.switchView(state.returnTo);
        return;
    }

    // If we are already on the home / dashboard state → show exit modal
    if (!state || state.view === 'view-owner') {
        showExitModal();
        // Immediately re-push the home state so the stack isn't empty
        // (prevents a second back press from bypassing the modal)
        window.history.pushState({ view: 'view-owner' }, '', window.location.pathname);
        return;
    }

    // Fallback: go to owner dashboard
    window.switchView(state.view || 'view-owner');
});
...
window.confirmExitApp = function() {
    // Remove the guard state we pushed so the real back navigates away
    window.history.go(-2);
};
