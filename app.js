...
window.handleAddBuildingRoom = async function(e) {
    ...
    try {
        ...
        // Create rooms for this building — each with a unique connectionCode
        for (let i = 0; i < roomCount; i++) {
            ...
            await addDoc(collection(db, "rooms"), {
                ...
                status:         "pending",
                ownerId:        currentUser.uid,
                connectionCode: generateConnectionCode(),   // ← unique 6-char code
                createdAt:      new Date()
            });
        }
...
window.renderRoomsList = function() {
    ...
    Object.entries(groupedByBuilding).forEach(([buildingId, rooms]) => {
        ...
        const roomCardsHTML = rooms.map((room) => {
                    ...
                    return `
                    <div class="room-card-grid bg-white rounded-2xl border-2 ${borderColor} p-3 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 relative">
                        <!-- Edit/Delete Room icons -->
                        ...
                        <div class="w-full mb-2">
                            <div class="w-full aspect-square ${avatarBg} rounded-xl flex items-center justify-center text-white font-black text-xl shadow-md">
                                ${avatarContent}
                            </div>
                        </div>
                        ...
                        <!-- Verification meta chip -->
                        ${isPendingVerify ? `<div class="text-center mb-1">
                            <span class="text-[10px] font-bold bg-purple-50 text-purple-600 px-1.5 py-0.5 rounded-md">via ${verifyApp}${verifyTimeStr ? ' · ' + verifyTimeStr : ''}</span>
                        </div>` : ''}
                        ...
                        <div class="flex flex-col gap-1">
                            ${isPendingVerify ? `
                            <button onclick="approvePaymentVerification('${room.id}', ${verifyAmount})" class="w-full py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-[11px] font-bold rounded-lg transition-all active:scale-95 btn-premium">
                                ✓ Verify Pay
                            </button>
                            <button onclick="rejectPaymentVerification('${room.id}')" class="w-full py-1.5 bg-gray-100 text-gray-600 hover:bg-red-100 hover:text-red-700 text-[11px] font-bold rounded-lg transition-all active:scale-95">
                                ✗ Reject
                            </button>
                            ` : `
...
window.submitKyc = async function() {
    ...
    // Generate a fresh 6-char connection code for this room
    ...
    try {
        await updateDoc(doc(db, "rooms", kycRoomId), {
            ...
            idProof:        idproof,
            status:         "pending",
            connectionCode: code    // ← save code to Firestore
        });
        showToast(`👤 Tenant assigned! Code: ${code}`, "success");

        // Show the code to the owner so they can share it
        showConnectionCodeModal(code, name);
...
/**
 * Generate a random 6-char alphanumeric code like "A4X9K2".
...
/**
 * Show a modal/toast displaying the newly generated connection code,
 * so the owner can share it immediately with the tenant.
 */
function showConnectionCodeModal(code, tenantName) {
    // Build a pretty display code like "A4X-9K2"
    const display = code.slice(0, 3) + '-' + code.slice(3);

    // Remove any existing one
    document.getElementById('code-reveal-modal')?.remove();

    const el = document.createElement('div');
    el.id = 'code-reveal-modal';
    el.className = 'fixed inset-0 z-[99] flex items-center justify-center px-5';
    el.innerHTML = `
        <div class="blur-background" onclick="document.getElementById('code-reveal-modal').remove()"></div>
        <div class="relative z-10 bg-white rounded-3xl p-8 w-full max-w-xs text-center shadow-2xl"
             style="border:2px solid #FED7AA;animation:scaleIn .4s cubic-bezier(0.34,1.56,.64,1)">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
                 style="background:linear-gradient(135deg,#FF6600,#F59E0B)">
                <i class="fa-solid fa-key text-3xl text-white"></i>
            </div>
            <h3 class="text-xl font-black mb-1" style="color:var(--indigo)">Connection Code</h3>
            <p class="text-sm text-gray-400 mb-5">${tenantName} के लिए · for ${tenantName}</p>

            <!-- Big code display -->
            <div class="py-4 px-6 rounded-2xl mb-5" style="background:linear-gradient(135deg,#1E1B4B,#4C1D95)">
                <p class="font-mono font-black text-4xl tracking-[0.25em] text-white">${display}</p>
            </div>

            <p class="text-xs text-gray-500 mb-5">
                यह code tenant को दें ताकि वो<br>app में join कर सके। <br>
                <span style="color:var(--saffron);font-weight:700">Share this code with your tenant.</span>
            </p>

            <div class="flex gap-3">
                <button onclick="shareCodeWhatsApp('${code}','${tenantName}')"
                        class="flex-1 py-3 rounded-2xl font-bold text-sm text-white btn-premium flex items-center justify-center gap-2"
                        style="background:linear-gradient(135deg,#22C55E,#16A34A)">
                    <i class="fa-brands fa-whatsapp"></i> Share
                </button>
                <button onclick="copyCode('${code}')"
                        class="flex-1 py-3 rounded-2xl font-bold text-sm btn-premium"
                        style="background:#F3E8D0;color:var(--indigo)">
                    <i class="fa-regular fa-copy mr-1"></i> Copy
                </button>
            </div>
            <button onclick="document.getElementById('code-reveal-modal').remove()"
                    class="mt-3 text-sm font-medium" style="color:#9CA3AF">
                Close
            </button>
        </div>`;
...
window.shareCodeWhatsApp = function(code, tenantName) {
    const display = code.slice(0,3) + '-' + code.slice(3);
    const msg =
`🏠 *Room Khata Pro* – Connection Code
───────────────────
Tenant: ${tenantName}
Code: *${display}*
───────────────────
👉 Download app → [link]
1. Click "किरायेदार" (Tenant)
2. Enter your WhatsApp number
3. Enter code: *${display}*

किराया आसानी से देखें और भरें! 🙏`;
    window.open('https://wa.me/?text=' + encodeURIComponent(msg), '_blank');
}
...
window.copyCode = async function(code) {
    ...
    try {
        ...
        showToast('✓ Code copied: ' + display, 'success');
    } catch(e) {
...
// ==========================================
// LOGIN ROLE SELECTOR  ─  UI helpers
// ==========================================

/**
 * Show role-selection panel, hide the two form panels.
 */
window.showLoginRoles = function() {
    document.getElementById('login-panel-roles').classList.remove('hidden');
    document.getElementById('login-panel-owner').classList.add('hidden');
    document.getElementById('login-panel-tenant').classList.add('hidden');
}

/**
 * Called when a role card is tapped.
 * @param {'owner'|'tenant'} role
 */
window.selectLoginRole = function(role) {
    // Animate the card selection briefly
    const cards = document.querySelectorAll('.role-card');
    cards.forEach(c => c.classList.remove('selected'));

    // Hide role panel → slide in correct form
    setTimeout(() => {
        document.getElementById('login-panel-roles').classList.add('hidden');
        if (role === 'owner') {
            const panel = document.getElementById('login-panel-owner');
            panel.classList.remove('hidden');
            // Slide-in animation
            panel.style.opacity = '0';
            panel.style.transform = 'translateX(40px)';
            setTimeout(() => {
                panel.style.transition = 'opacity .4s cubic-bezier(0.34,1.56,.64,1), transform .4s cubic-bezier(0.34,1.56,.64,1)';
                panel.style.opacity = '1';
                panel.style.transform = 'translateX(0)';
            }, 10);
            setTimeout(() => document.getElementById('auth-email')?.focus(), 300);
        } else {
            const panel = document.getElementById('login-panel-tenant');
            panel.classList.remove('hidden');
            panel.style.opacity = '0';
            panel.style.transform = 'translateX(40px)';
            setTimeout(() => {
            ...
            setTimeout(() => document.getElementById('tenant-phone')?.focus(), 300);
        }
    }, 120);
}

// ==========================================
// 6-BOX CODE INPUT  ─  UX helpers
// ==========================================

window.codeBoxInput = function(index, el) {
    // Sanitise: keep only alphanumeric, uppercase
    el.value = el.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().slice(-1);
    el.classList.toggle('filled', el.value !== '');
    // Auto-advance
    if (el.value && index < 5) {
        const next = index < 2 ? index + 1 : index + 1; // skip separator
        document.getElementById('cb' + (index + 1))?.focus();
    }
}

window.codeBoxKeydown = function(index, e) {
...
/**
 * Read all 6 code-box inputs and return the assembled code string.
 */
function getEnteredCode() {
    return [0,1,2,3,4,5].map(i => (document.getElementById('cb' + i)?.value || '')).join('').toUpperCase();
}

/**
 * Clear all code boxes (called on failed attempt).
 */
...
/**
 * handleTenantCodeLogin — reads phone + 6 code boxes,
 * queries Firestore for matching connectionCode,
 * signs in anonymously, links tenantUid to the room.
 */
window.handleTenantCodeLogin = async function() {
    const phone = document.getElementById('tenant-phone').value.trim();
    const code  = getEnteredCode();

    if (!phone || phone.length < 10) {
        showToast("Valid WhatsApp number (10 digits) डालें", "error");
        document.getElementById('tenant-phone')?.focus();
        return;
    }
    if (code.length < 6) {
        showToast("6-digit code पूरा डालें / Enter complete 6-digit code", "error");
        document.getElementById('cb0')?.focus();
        return;
    }

    const btn     = document.getElementById('tenant-login-btn');
    const btnText = document.getElementById('tenant-btn-text');
    const ogText  = btnText.innerHTML;
    btnText.innerHTML = '<span class="spinner mr-2"></span> Searching...';
    btn.disabled = true;

    try {
        // ── Step 1: Find the room with this connectionCode ──
        ...
        if (snap.empty) {
            showToast("❌ Invalid code. अपने owner से code check करें.", "error");
            clearCodeBoxes();
            btnText.innerHTML = ogText;
            btn.disabled = false;
            return;
        }
        ...
        btnText.innerHTML = '<span class="spinner mr-2"></span> Connecting...';
        const cred       = await signInAnonymously(auth);
        const tenantUid  = cred.user.uid;

        // ── Step 3: Link UID + phone to the room ──
        await updateDoc(doc(db, "rooms", roomId), {
            tenantPhone: '+91' + phone,
            tenantUid:   tenantUid,
            status:      roomData.status === 'pending' ? 'pending' : roomData.status
        });

        // ── Step 4: Save/merge tenant profile ──
        await setDoc(doc(db, "tenantProfiles", tenantUid), {
            phone:     '+91' + phone,
            roomId:    roomId,
            joinedAt:  new Date().toISOString()
        ...
        showToast("🎉 Successfully connected! Welcome!", "success");
        // onAuthStateChanged will detect tenantUid → view-tenant-dashboard
    } catch (err) {
        let msg = err.message;
        if (err.code === 'permission-denied') msg = "Permission error. Contact your owner.";
...
window.handleLoginSubmit = async function(e) {
    ...
    const ogText = btnText ? btnText.innerHTML : 'Sign In / साइन इन';
    if (btnText) btnText.innerHTML = '<span class="spinner mr-2"></span> Signing in...';
    btn.disabled = true;
    ...
    try {
        ...
        if (err.code === 'auth/network-request-failed') msg = "Network error. Check your connection.";
        showToast(msg, "error");
        if (btnText) btnText.innerHTML = ogText;
        btn.disabled = false;
    }
}
